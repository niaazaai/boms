...
**Verdict:** the shape is right (procurement → inventory → sales → finance), but the spec is **not yet buildable**. 43 defects below; 12 are blocking.
...
### G5 — RTL is asserted, never drawn beyond one frame · **S3**
Two frames demonstrate RTL. With Dari/Pashto as primary locales, every mirrored screen needs at least a documented rule set.
